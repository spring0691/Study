...
hidden_list = [1,2,4,8,16,32]

for hidden_size in hidden_list:
    locals()[f'model_{hidden_size:02d}'] = autoencoder(hidden_layer_size=hidden_size)
    
    print(f"############################## node {hidden_size}개 시작 ########################")

    model_.compile(optimizer='adam', loss='binary_crossentropy')
    model_.fit(x_train,x_train,epochs=10)
    globals()[f'output_{hidden_size:02d}'] = model_.predict(x_test)
