...
'''
parameters = {'max_depth' : [3],'eta': [0.1],'objective':['reg:linear'],'eval_metric':['logloss'],'early_stopping':[100]}
model = RandomizedSearchCV(XGBRegressor(), parameters, cv=kfold, n_jobs=-1, n_iter = 20, random_state=66)
#model = GridSearchCV(LGBMRegressor(), parameters, cv=kfold, n_jobs=-1)
#model = GridSearchCV(CatBoostRegressor(),  cv=kfold, n_jobs=-1)

print(XGBRegressor().get_params().items())
'''
...
'''
나는 회귀모델!
최적의 매개변수는요~ :  RandomForestRegressor(max_depth=10, min_samples_leaf=3, min_samples_split=5,n_estimators=200)
