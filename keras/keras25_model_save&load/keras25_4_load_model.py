...
# 그래서 load기능을 이용해서 모델을 불러오면 그거 하나만으로 모델을 돌릴수가 있다. 이제 여기서 저장시점이 중요해지는데
# model.save를 모델 다음에 하면모델까지만 저장하고 fit다음에 하면 weight까지 저장된다.
# 모델이라는게 레이어 + 히든레이어 값들 + epoch와 batchsize + validation 등등의 모든값이 다 담기는거 같다.